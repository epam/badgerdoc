...
helm install bagerdoc-keycloack -n dev1 -f values.yaml --set  externalDatabase.password=testpassword --set externalDatabase.user=postgres  bitnami/keycloak

name "bagerdoc-keycloack" will be overrided to "bagerdoc-keycloack" from values.yaml
```

# database
there is two database backups with structure and test data (added admin user and data)
keykloack.sql - sql format database backup
keykloack backup - pgdump backup
