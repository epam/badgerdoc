...
- [ ] **README.md** — `workflows/badgerdoc_ocr_<tag>/README.md` created; for MLX-backed engines use the three-section format from `badgerdoc_ocr_deepseek_2/README.md` (install `mlx-vlm`, start server with correct port + model, verify with `curl http://localhost:<PORT>/health`); for non-MLX engines a one-paragraph description is sufficient
