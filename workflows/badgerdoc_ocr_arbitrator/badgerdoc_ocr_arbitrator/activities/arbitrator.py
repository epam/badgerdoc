...
async def get_existing_workflows(
    ...
    workflows = await workflow_registry.badgerdoc_get_workflows(
        params
    )
    if workflows:
        return workflows
...
async def start_arbitrator(
    ...
    for workflow_data in workflows:
        ...
        for document_id in sorted(document_ids):
            endpoint = (
                f"/badgerdoc/workflow-registry/manual-trigger/{workflow_data.id}/"
            )
            payload = {
            ...
            if isinstance(workflow_input_data, dict):
                target_extraction = workflow_input_data.get("target_extraction")
