...
from badgerdoc_ocr_deepseek_2.deepseek2_ocr import Deepseek2OCR
...
class BadgerdocDeepseek2Workflow:
    ...
    async def run(
        ...
        ocr_page_container = await trigger_params_to_ocr_page(params)
        logger.info(
            "BadgerdocDeepseek2Workflow: OCR container resolved — %d pages, %d blocks",
            len(ocr_page_container.pages),
            len(ocr_page_container.blocks),
        )

        hocr_results = await Deepseek2OCR().run(params, ocr_page_container)
