...
def get_taxonomy(
    ...
    if taxonomy.tenant in (tenant, None):
        ...
        raise CheckFieldError("Wrong tenant")
...
def update_taxonomy_instance(
    ...
    for key, value in new_data.dict().items():
        if key == "id" and taxonomy.tenant in (None, tenant):
            continue
...
def get_second_latest_taxonomy(
    ...
    return (
        ...
        .filter(or_(Taxonomy.tenant == tenant, Taxonomy.tenant == null()))
...
def create_new_relation_to_job(
    session: Session, taxonomy: Taxonomy, job_id: str, tenant: str,
) -> None:
    if taxonomy.tenant in (tenant, None):
...
def get_taxonomies_by_job_id(
        session: Session, job_id: str, tenant: str,) -> List[Taxonomy]:
